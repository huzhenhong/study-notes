 ...
 * LastEditTime : 2022-06-24 11:44:53
 * FilePath     : \\cplusplus\\src\\condition-variables-and-semaphore\\one-producer-and-one-consumer.cpp
...
void                    Producer(int threadID)
{
    ...
    // while (!g_bFinished)
    {
        ...
        g_cv.wait(lk, []()
                  { return g_dataDeq.size() <= MAX_SIZE; });  // 队列大小大于 MAX_SIZE 时释放锁，直到队列大小不大于 MAX_SIZE 时重新获得锁并继续执行
